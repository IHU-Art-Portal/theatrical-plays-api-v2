...
public class ContributionValidationService : IContributionValidationService
{
    ...
    {
        var report = new ValidationReport();
        var existence = await _repository.CheckExists(contributionDto.PerformerId, contributionDto.ProductionId, contributionDto.RoleId);

        if (!existence.performerExists)
        {
            report.Success = false;
            report.Message = $"Performer with Id: {contributionDto.PerformerId} not found";
            return report;
        }
